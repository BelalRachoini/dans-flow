-- Update check_in_with_qr to support self-check-in by members
...
BEGIN
  ...
  ELSE
    v_is_self_checkin := (v_ticket.member_id = auth.uid());
  END IF;
  ...
  -- Handle course ticket (original logic with self check-in support)
  ...
  IF v_ticket.checked_in_count >= v_ticket.max_checkins THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'ALREADY_CHECKED_IN',
      'message', 'Biljetten har använt alla sina incheckningar'
    );
  END IF;
  ...
  -- Verify instructor has access (unless admin or self check-in)
  IF NOT v_is_self_checkin AND v_scanner_role = 'instructor' AND v_course.instructor_id != auth.uid() THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'UNAUTHORIZED',
