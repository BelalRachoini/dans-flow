...
-- Drop problematic policies that use has_role() recursively
DROP POLICY IF EXISTS "Admins can view all roles" ON public.user_roles;
DROP POLICY IF EXISTS "Admins can insert roles" ON public.user_roles;
DROP POLICY IF EXISTS "Admins can update roles" ON public.user_roles;
DROP POLICY IF EXISTS "Admins can delete roles" ON public.user_roles;

-- Create non-recursive policies with direct EXISTS queries
...
-- Update check_in_with_qr function to query user_roles instead of profiles.role
...
BEGIN
  -- Get scanner role from user_roles table (FIXED: was querying profiles.role)
