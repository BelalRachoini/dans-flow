...
import { Ticket, Check, CreditCard, Smartphone } from 'lucide-react';
...
export function StandaloneTicketPurchaseDialog({ open, onOpenChange }: StandaloneTicketPurchaseDialogProps) {
  ...
  const handlePurchase = async (ticketCount: number, price: number) => {
    setPurchasing(true);
    try {
        ...
        const { data, error } = await supabase.functions.invoke('create-swish-payment', {
          body: {
            ...
            metadata: {
              ...
              message: `Klippkort ${ticketCount} st`,
            },
          },
        });
              ...
              <div key={pkg.count} className="relative rounded-lg border-2 p-6 hover:border-primary transition-colors">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <Button
                      onClick={() => handlePurchase(pkg.count, pkg.price)}
                      disabled={purchasing}
