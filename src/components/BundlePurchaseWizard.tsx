...
export function BundlePurchaseWizard({ courseId, courseName }: BundlePurchaseWizardProps) {
  ...
  useEffect(() => { loadTiers(); }, [courseId]);
  useEffect(() => { if (selectedTier) loadAllowedClasses(); }, [selectedTier]);

  const loadTiers = async () => {
    try {
      const { data, error } = await supabase
        .from('course_bundle_tiers' as any).select('*')
  ...
  const loadAllowedClasses = async () => {
    ...
    if (selectedTier.class_filter_mode === 'all') {
      setAllowedClassIds(null);
    } else {
      const { data, error } = await supabase
        .from('course_bundle_tier_classes' as any).select('class_id')
        .eq('tier_id', selectedTier.id);
      if (error) { setAllowedClassIds([]); } else { setAllowedClassIds((data as any[])?.map(tc => tc.class_id) || []); }
    }
  };

  const handleSelectTier = (tier: BundleTier) => {
    setSelectedTier(tier); setSelectedClassIds([]); setSelectedClassNames([]); setStep('classes');
  };

  const handleClassSelectionChange = async (classIds: string[]) => {
    setSelectedClassIds(classIds);
    if (classIds.length > 0) {
      const { data } = await supabase.from('course_classes').select('id, name').in('id', classIds);
      setSelectedClassNames(data?.map(c => c.name) || []);
    } else { setSelectedClassNames([]); }
  };
  ...
  const handleProceedToCheckout = async () => {
    ...
    try {
      ...
      if (!session) { toast.error(t.auth?.pleaseLogin || 'Please log in to purchase'); return; }

      if (paymentMethod === 'swish') {
        const { data, error } = await supabase.functions.invoke('create-swish-payment', {
          body: {
            payment_type: 'course',
            amount_sek: selectedTier.price_cents / 100,
            metadata: {
              course_id: courseId,
              tier_id: selectedTier.id,
              selected_class_ids: JSON.stringify(selectedClassIds),
              is_package: "true",
              is_bundle: "true",
              message: courseName.substring(0, 50),
            },
          },
        });
        if (error) throw error;
        ...
        const { data, error } = await supabase.functions.invoke('create-course-payment', {
          body: { course_id: courseId, tier_id: selectedTier.id, selected_class_ids: selectedClassIds },
        });
        if (error) throw error;
    ...
    <>
        ...
        {step === 'tier' && (
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {tiers.map((tier) => (
                <Card key={tier.id} className={`cursor-pointer transition-all hover:border-primary ${tier.is_popular ? 'border-primary ring-2 ring-primary/20' : ''}`} onClick={() => handleSelectTier(tier)}>
                  <CardContent className="p-6 text-center relative">
                    {tier.is_popular && <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 gap-1"><Star className="h-3 w-3" /> Populär</Badge>}
                    <h3 className="text-lg font-bold mt-2">{tier.name}</h3>
                    <p className="text-3xl font-bold text-primary my-4">{(tier.price_cents / 100).toLocaleString('sv-SE')} kr</p>
                    <p className="text-muted-foreground mb-4">{tier.max_selections} {tier.max_selections === 1 ? 'klass' : 'klasser'}</p>
                    <Button variant={tier.is_popular ? 'default' : 'outline'} className="w-full">Välj</Button>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        {step === 'classes' && selectedTier && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Button variant="ghost" onClick={goBack}><ChevronLeft className="h-4 w-4 mr-1" /> Tillbaka</Button>
              <div className="text-center">
                <h2 className="text-xl font-semibold">Välj dina klasser</h2>
                <p className="text-sm text-muted-foreground">{selectedTier.name} - välj upp till {selectedTier.max_selections} klasser</p>
              </div>
              <div className="w-24" />
            </div>
            <PackageClassSelector courseId={courseId} maxSelections={selectedTier.max_selections} selectedClassIds={selectedClassIds} onSelectionChange={handleClassSelectionChange} allowedClassIds={allowedClassIds} />
            <div className="flex justify-end">
              <Button onClick={goNext} disabled={selectedClassIds.length === 0} className="gap-2">Fortsätt <ChevronRight className="h-4 w-4" /></Button>
            </div>
          </div>
        )}
            ...
            <Card>
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between pb-4 border-b">
                  <div><p className="font-semibold">{courseName}</p><p className="text-sm text-muted-foreground">{selectedTier.name}</p></div>
                  <Badge variant="outline">{selectedClassIds.length} {selectedClassIds.length === 1 ? 'klass' : 'klasser'}</Badge>
                </div>
                <div>
                  <p className="text-sm font-medium mb-2">Valda klasser:</p>
                  <ul className="space-y-1">
                    {selectedClassNames.map((name, idx) => (
                      <li key={idx} className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="h-3 w-3 text-primary" />{name}</li>
                    ))}
                  </ul>
                </div>
                <div className="flex items-center justify-between pt-4 border-t">
                  <p className="font-semibold">Totalt</p>
                  <p className="text-2xl font-bold text-primary">{(selectedTier.price_cents / 100).toLocaleString('sv-SE')} kr</p>
                </div>
              </CardContent>
            </Card>
      ...
      {swishPaymentId && (
        <SwishPaymentStatus
          ...
          onSuccess={() => { toast.success('Betalning klar!'); setSwishPaymentId(null); }}
        />
      )}
    </>
  );
