...
export const LessonBookingDialog = ({ open, onOpenChange, lesson }: LessonBookingDialogProps) => {
  ...
  const fetchAvailableTickets = async () => {
    ...
    try {
      ...
      if (tickets) {
        const available = tickets.reduce((sum, ticket) => {
          return sum + (ticket.total_tickets - ticket.tickets_used);
        }, 0);
  ...
  const handleUseExistingTicket = async () => {
    ...
    try {
      ...
      // Create booking with existing ticket
      const { data, error } = await supabase
        .from("lesson_bookings")
        .insert({
          member_id: user.id,
          lesson_id: lesson.id,
          ticket_type: 'existing',
          checkins_allowed: 1,
          status: 'valid',
        })
        .select()
          ...
          <div className="space-y-3 rounded-lg border border-border bg-muted/50 p-4">
            ...
            <div className="flex items-start gap-2 text-sm">
              ...
              <span>
                ...
                {lessonEndTime && ` - ${format(lessonEndTime, "HH:mm")}`}
              </span>
            </div>
          ...
          {checkingTickets ? (
            ...
            <div className="space-y-3">
              {availableTickets > 0 && (
                <div className="flex items-center justify-between rounded-lg border border-border bg-background p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">{t.lessonBooking.availableTickets}</span>
                  </div>
              ...
              <div className="space-y-2">
                {availableTickets > 0 && (
                  <Button
                    onClick={handleUseExistingTicket}
                    disabled={loading}
                    className="w-full"
                    variant="default"
                  >
