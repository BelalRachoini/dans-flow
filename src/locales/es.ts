export const es = {
  // Navigation
  nav: {
    oversikt: 'Resumen',
    kurserPoang: 'Cursos',
    schema: 'Horario',
    event: 'Eventos',
    biljetter: 'Entradas',
    butik: 'Tienda',
    medlemmar: 'Miembros',
    prenumerationer: 'Suscripciones',
    betalningar: 'Pagos',
    rapporter: 'Informes',
    admin: 'Admin',
    courses: 'Cursos',
    events: 'Eventos',
  },

  // Auth
  auth: {
    login: 'Iniciar sesión',
    register: 'Registrarse',
    logout: 'Cerrar sesión',
    email: 'Correo electrónico',
    password: 'Contraseña',
    name: 'Nombre',
    phone: 'Teléfono',
    pleaseLogin: 'Por favor inicia sesión para inscribirte',
    loginRequired: 'Debes iniciar sesión',
    forgotPassword: '¿Olvidaste tu contraseña?',
    resetPassword: 'Restablecer contraseña',
    newPassword: 'Nueva contraseña',
    confirmPassword: 'Confirmar contraseña',
    sendResetLink: 'Enviar enlace de restablecimiento',
    resetLinkSent: 'Se ha enviado un enlace de restablecimiento a tu correo.',
    checkEmail: 'Revisa tu correo',
    passwordResetSuccess: '¡Contraseña restablecida!',
    backToLogin: 'Volver al inicio de sesión',
    passwordsDoNotMatch: 'Las contraseñas no coinciden',
    enterEmailForReset: 'Ingresa tu correo electrónico y te enviaremos un enlace de restablecimiento.',
    updatePassword: 'Actualizar contraseña',
  },

  // Lesson Booking
  lessonBooking: {
    title: 'Reservar Lección',
    description: 'Elige cómo quieres reservar esta lección',
    lessonDetails: 'Detalles de la Lección',
    availableTickets: 'Tickets Disponibles',
    useTicket: 'Usar 1 Ticket',
    or: 'o',
    buyDropIn: 'Comprar Drop-in',
    single: '1 ticket drop in',
    couple: '2 tickets drop in',
    trio: '3 tickets drop in',
    bookingSuccess: '¡Reserva exitosa!',
    bookingError: 'No se pudo crear la reserva',
    paymentError: 'No se pudo crear el pago',
  },

  // Common
  common: {
    save: 'Guardar',
    cancel: 'Cancelar',
    delete: 'Eliminar',
    edit: 'Editar',
    create: 'Crear',
    search: 'Buscar',
    filter: 'Filtrar',
    close: 'Cerrar',
    loading: 'Cargando...',
    noData: 'No hay datos disponibles',
    error: 'Ocurrió un error',
    success: 'Acción exitosa',
    done: 'Hecho',
    duplicate: 'Duplicar',
    duplicating: 'Duplicando...',
    duplicated: '¡Duplicado!',
    giveTickets: 'Dar entradas',
    ticketCount: 'Número de entradas',
    ticketExpiry: 'Fecha de vencimiento',
    ticketsGiven: '¡{count} entradas dadas!',
    removeTickets: 'Eliminar entradas',
    ticketsRemoved: '¡{count} entradas eliminadas!',
    notEnoughTickets: 'El miembro no tiene suficientes entradas',
    optional: 'opcional',
  },

  // Image Upload
  imageUpload: {
    courseImage: 'Imagen del Curso',
    uploadTab: 'Subir',
    urlTab: 'URL',
    uploadInstructions: 'Clic para subir o arrastra y suelta',
    maxFileSize: 'Máx 5MB • JPG, PNG, WebP',
    removeImage: 'Eliminar imagen',
    imagePreview: 'Vista previa',
    invalidType: 'Solo se permiten imágenes JPG, PNG y WebP',
    fileTooLarge: 'El archivo debe ser menor de 5MB',
    uploadSuccess: 'Imagen subida exitosamente',
    uploadError: 'Error al subir imagen',
    invalidUrl: 'Formato de URL inválido',
  },

  // Dashboard
  dashboard: {
    welcome: 'Bienvenido de nuevo',
    todaysSchedule: 'Horario de hoy',
    myPoints: 'Mis puntos',
    quickStats: 'Estadísticas rápidas',
    recentPayments: 'Pagos recientes',
    quickActions: 'Acciones rápidas',
    overview: 'Resumen',
    overviewDescription: '¡Bienvenido de nuevo! Aquí está tu resumen.',
    myClassesToday: 'Mis clases de hoy',
    scheduledClassesToday: 'Tus clases programadas para hoy',
    noClassesToday: 'No hay clases hoy',
    myTickets: 'Mis entradas',
    recentActiveTickets: 'Entradas activas recientes',
    viewAllTickets: 'Ver todas las entradas',
    noActiveTickets: 'No hay entradas activas',
    buyCourse: 'Comprar curso',
    shortcuts: 'Atajos',
    shortcutsDescription: 'Acceso rápido a tus funciones más importantes',
    upcoming: 'Próximos',
    upcomingDescription: 'Próximas lecciones y eventos',
    noUpcoming: 'No hay lecciones o eventos próximos',
    myBookings: 'Mis Reservas',
    myBookingsDescription: 'Tus lecciones reservadas',
    noBookings: 'No hay lecciones reservadas',
    viewHistory: 'Ver historial →',
    bookNow: 'Reservar Ahora',
    myEvents: 'Mis Eventos',
    myEventsDescription: 'Eventos para los que tienes entradas',
    noEvents: 'No hay reservas de eventos próximos',
    tickets: 'entradas',
    myTicketPackages: 'Mis Paquetes de Entradas',
    myTicketPackagesDescription: 'Tus paquetes de entradas activos',
    noTicketPackages: 'No hay paquetes de entradas activos',
    ticketsRemaining: 'entradas restantes',
    expires: 'Expira',
    viewAll: 'Ver todo',
    expired: 'Expirado',
  },

  // Courses & Points
  courses: {
    title: 'Cursos',
    subtitle: '',
    buyPoints: 'Comprar puntos',
    ticketsRemaining: 'Entradas Restantes',
    pointsHistory: 'Historial de puntos',
    totalLessons: 'Total de lecciones',
    checkIn: 'Registrarse',
    book: 'Reservar',
    viewSchedule: 'Ver Horario',
    flexiblePoints: '¡Puntos flexibles - úsalos en cualquier clase!',
    createCourse: 'Crear Curso',
    courseTitle: 'Título del Curso',
    style: 'Estilo',
    startDate: 'Fecha de Inicio',
    endDate: 'Fecha de Fin',
    dayOfWeek: 'Día de la Semana',
    time: 'Hora',
    location: 'Ubicación',
    description: 'Descripción',
    price: 'Precio (SEK)',
    instructor: 'Instructor',
    instructors: 'Instructores',
    selectInstructors: 'Seleccionar instructores',
    selectedInstructors: 'seleccionados',
    primaryInstructor: 'Instructor Principal',
    selectPrimaryInstructor: 'Seleccionar instructor principal',
    noPrimary: 'Sin instructor principal',
    primaryInstructorHelp: 'Opcional: Seleccione un instructor principal de los seleccionados',
    createSuccess: '¡El curso ha sido creado!',
    sunday: 'Domingo',
    monday: 'Lunes',
    tuesday: 'Martes',
    wednesday: 'Miércoles',
    thursday: 'Jueves',
    friday: 'Viernes',
    saturday: 'Sábado',
    formDescription: 'Completar información del curso',
    selectInstructor: 'Seleccionar instructor',
    noInstructor: 'Ninguno',
    startDateTime: 'Fecha y hora de inicio',
    endDateTime: 'Fecha y hora de fin',
    selectDateTime: 'Seleccionar fecha y hora',
    timeLabel: 'Hora',
    lessonsCount: 'lecciones',
    priceLabel: 'Precio:',
    pointsLabel: 'Puntos:',
    errorLoad: 'No se pudieron cargar los cursos',
    errorSave: 'No se pudo guardar el curso',
    errorDelete: 'No se pudo eliminar el curso',
    deleteConfirmation: 'Esto no se puede deshacer. El curso y todas sus lecciones se eliminarán permanentemente.',
    upcomingEventsTitle: '¡No te pierdas nuestros próximos eventos!',
    upcomingEventsDescription: 'Desde bailes sociales hasta talleres y noches especiales: descubre todos los emocionantes eventos que tenemos para ofrecer.',
    viewAllEvents: 'Ver todos los eventos',
    lessons: {
      manual: 'Manual',
      recurring: 'Recurrente',
      dayOfWeek: 'Día de la Semana',
      startTime: 'Hora de Inicio',
      endTime: 'Hora de Fin',
      generateLessons: 'Generar Clases',
      lessonsWillBeCreated: 'Esto creará {count} clases',
      titleTemplate: 'Clase {n}',
      lessonsGenerated: '{count} clases generadas',
      needCourseDates: 'El curso debe tener fechas de inicio y fin',
      endTimeAfterStart: 'La hora de fin debe ser después de la hora de inicio',
    },
    pageEditor: {
      editPage: 'Editar Página',
      addSection: 'Agregar Sección',
      saveSection: 'Guardar Sección',
      deleteSection: 'Eliminar Sección',
      moveUp: 'Mover Arriba',
      moveDown: 'Mover Abajo',
      sectionType: 'Tipo de Sección',
      sectionTitle: 'Título de Sección (opcional)',
      confirmDelete: '¿Estás seguro de que quieres eliminar esta sección?',
      hero: 'Hero (portada)',
      text: 'Texto',
      video: 'Video',
      image: 'Imagen',
      gallery: 'Galería',
      faq: 'FAQ',
      booking: 'Inscripción',
      heroDescription: 'La sección hero usará automáticamente el título, imagen, nivel, instructor y fechas del curso.',
      textContent: 'Contenido de Texto',
      textPlaceholder: 'Ingresa contenido de texto...',
      textAlignment: 'Alineación de Texto',
      fontSize: 'Tamaño de Fuente',
      videoUrl: 'URL de Video (YouTube/Vimeo embed)',
      aspectRatio: 'Relación de Aspecto',
      imageUrl: 'URL de Imagen',
      altText: 'Texto Alt',
      caption: 'Leyenda (opcional)',
      galleryImages: 'Imágenes',
      addImage: 'Agregar Imagen',
      faqItems: 'Elementos FAQ',
      question: 'Pregunta',
      answer: 'Respuesta',
      removeItem: 'Eliminar',
      addFaqItem: 'Agregar Elemento FAQ',
      bookingDescription: 'La sección de inscripción mostrará automáticamente el precio del curso, información de tickets, horario de clases y botón de inscripción.',
      titlePlaceholder: 'Ingresa título de sección...',
      editSection: 'Editar Sección',
    },
  },

  // Events
  events: {
    title: 'Eventos',
    createEvent: 'Crear evento',
    editEvent: 'Editar evento',
    deleteEvent: 'Eliminar evento',
    deleted: 'Evento eliminado',
    saved: 'Evento guardado',
    eventTitle: 'Título',
    imageUrl: 'URL de imagen',
    date: 'Fecha',
    time: 'Hora',
    description: 'Descripción',
    venue: 'Lugar',
    price: 'Precio',
    capacity: 'Capacidad',
    soldOut: 'Agotado',
    ended: 'Finalizado',
    ticketsAvailable: 'Entradas disponibles',
    buyTicket: 'Comprar entrada',
    noEvents: 'Aún no hay eventos',
    upcoming: 'Próximos',
    past: 'Pasados',
    eventDetails: 'Detalles del evento',
    tickets: 'Entradas',
    ticketTypes: 'Tipos de entradas',
    generalAdmission: 'General',
    vip: 'VIP',
    viewAttendees: 'Ver asistentes',
    attendees: 'Asistentes',
    attendeesList: 'Lista de asistentes',
    noAttendees: 'Aún no hay asistentes',
    bookedAt: 'Reservado',
    paymentStatus: 'Estado de pago',
    bookingStatus: 'Estado',
    discount: {
      add: 'Agregar descuento',
      type: 'Tipo de descuento',
      percent: 'Porcentaje',
      amount: 'Cantidad',
      value: 'Valor',
    },
    status: 'Estado',
    statusDraft: 'Borrador',
    statusPublished: 'Publicado',
  },

  // Tickets
  tickets: {
    title: 'Entradas',
    explanation: {
      title: 'Cómo funcionan las tarjetas de punche',
      description: 'Cuando compras un curso, recibes tarjetas de punche que se pueden usar para TODAS las clases, no solo el curso que compraste. Cada clase usa 1 punche. Ten en cuenta que las tarjetas de punche tienen fecha de vencimiento basada en el período del curso. Las tarjetas de punche no se pueden usar para eventos.',
    },
    ticketPackage: 'Tarjeta de Punche',
    fromCourse: 'Del Curso',
    totalTickets: 'Punches Totales',
    usedTickets: 'Punches Usados',
    remainingTickets: 'Restantes',
    expiresAt: 'Vence',
    expired: 'Vencido',
    courseTickets: 'Tarjetas de Punche',
    eventTickets: 'Entradas de Eventos',
    dropInTickets: 'Entradas Drop-in',
    noTickets: '¡Aún no hay tarjetas de punche. Compra un curso para obtener tarjetas de punche!',
    noEventTickets: 'Aún no hay entradas de eventos.',
    purchasedOn: 'Comprado',
    showQR: 'Mostrar QR',
    checkIn: 'Registrarse',
    buy: 'Comprar entrada',
    myTickets: 'Mis entradas',
    buyCourses: 'Comprar cursos',
    viewEvents: 'Ver eventos',
    adminAttendance: 'Admin: Ver Asistencia',
    selectCourse: 'Seleccionar Curso',
    selectEvent: 'Seleccionar Evento',
    chooseCourse: 'Elige un curso...',
    chooseEvent: 'Elige un evento...',
    totalAttendees: 'Total Asistentes',
    leaders: 'Líderes',
    followers: 'Seguidores',
    notSet: 'No Configurado',
    checkedIn: 'Registrado',
    notCheckedIn: 'No Registrado',
    name: 'Nombre',
    email: 'Correo',
    phone: 'Teléfono',
    danceRole: 'Rol de Baile',
    ticketsRemaining: 'Entradas Restantes',
    checkInStatus: 'Estado de Registro',
    noAttendees: 'No se encontraron asistentes',
    buyTickets: 'Comprar Entradas',
  },

  // Memberships
  memberships: {
    title: 'Membresías y Suscripciones',
    plans: 'Planes de membresía',
    manage: 'Gestionar suscripción',
    start: 'Iniciar suscripción',
    pause: 'Pausar',
    resume: 'Reanudar',
    cancel: 'Cancelar',
    nextBilling: 'Próxima facturación',
    benefits: 'Beneficios',
  },

  // Payments
  payments: {
    title: 'Pagos',
    invoices: 'Facturas',
    receipts: 'Recibos',
    paymentMethods: 'Métodos de pago',
    stripe: 'Stripe',
    paypal: 'PayPal',
    total: 'Total',
    pay: 'Pagar',
  },

  // Members
  members: {
    title: 'Gestión de Miembros',
    addMember: 'Añadir miembro',
    viewProfile: 'Ver perfil',
    addPoints: 'Añadir puntos',
    pauseMembership: 'Pausar membresía',
    deactivate: 'Desactivar cuenta',
    profile: 'Perfil',
    attendanceStats: 'Estadísticas de asistencia',
  },

  // Reports
  reports: {
    title: 'Informes y Análisis',
    attendance: 'Asistencia',
    revenue: 'Ingresos',
    popularStyles: 'Estilos populares',
    ticketSales: 'Ventas de entradas',
    exportCSV: 'Exportar CSV',
  },

  // Admin
  admin: {
    title: 'Panel de Admin',
    contentManagement: 'Gestión de contenido',
    userManagement: 'Gestión de usuarios',
    communication: 'Comunicación',
    sendEmail: 'Enviar correo',
    sendSMS: 'Enviar SMS',
    sendWhatsApp: 'Enviar WhatsApp',
    recipients: 'Destinatarios',
    subject: 'Asunto',
    message: 'Mensaje',
    send: 'Enviar',
  },

  // Admin Dashboard
  adminDashboard: {
    title: 'Admin - Resumen',
    subtitle: 'Control completo del sistema',
    coursesThisWeek: 'Cursos esta semana',
    eventsThisWeek: 'Eventos esta semana',
    totalMembers: 'Total de miembros',
    activeTickets: 'Entradas activas',
    upcomingClasses: 'Clases y eventos próximos',
    nextWeek: 'Próxima semana',
    noUpcomingClasses: 'No hay clases próximas',
    quickActions: 'Acciones Rápidas',
    quickActionsDesc: 'Acceso rápido a tareas de administración comunes',
    scanQR: 'Escanear QR',
    createCourse: 'Crear curso',
    createEvent: 'Crear evento',
    openSchedule: 'Abrir Horario',
    manageMembers: 'Gestionar miembros',
    payments: 'Pagos',
    settings: 'Configuración',
  },

  // QR
  qr: {
    scan: 'Escanear QR',
    generate: 'Generar QR',
    checkedIn: 'Registrado',
    invalid: 'Código QR no válido',
    scanTitle: 'Escanear entradas',
    scanSubtitle: 'Registrar miembros en cursos',
    cameraTab: 'Escaneo con cámara',
    manualTab: 'Código manual',
    startCamera: 'Iniciar cámara',
    stopCamera: 'Detener',
    selectCamera: 'Seleccionar cámara',
    ticketCode: 'Código de entrada',
    enterCode: 'Pega o escribe el código de entrada...',
    checkInButton: 'Registrar',
    success: '¡Registro exitoso!',
    recentCheckIns: 'Registros recientes (hoy)',
    noCheckIns: 'No hay registros aún hoy',
    howItWorks: 'Cómo funciona:',
    howItWorksDesc: 'Usa la cámara para escanear los códigos QR de los miembros o ingresa el código manualmente. El sistema registra automáticamente la entrada y actualiza el estado de la entrada.',
    errorAlready: 'Entrada ya registrada',
    errorInvalid: 'Entrada inválida',
    errorInvalidStatus: 'La entrada no es válida',
    errorUnauthorized: 'Sin permisos',
    errorCamera: 'No se pudo iniciar la cámara. Verifica los permisos.',
    errorUnknown: 'Algo salió mal',
    noAccess: 'Debes ser instructor o administrador para escanear entradas',
  },

  // Roles
  roles: {
    ADMIN: 'Admin',
    INSTRUKTOR: 'Instructor',
    MEDLEM: 'Miembro',
  },

  // Dance Styles
  styles: {
    Salsa: 'Salsa',
    Bachata: 'Bachata',
    Tango: 'Tango',
    Kizomba: 'Kizomba',
    Zouk: 'Zouk',
    HipHop: 'HipHop',
  },

  // Language
  language: {
    title: 'Idioma',
    swedish: 'Sueco',
    english: 'Inglés',
    spanish: 'Español',
  },

  // Events
  event: {
    create: "Crear evento",
    edit: "Editar evento",
    delete: "Eliminar evento",
    deleted: "Evento eliminado",
    saved: "Evento guardado",
    title: "Título",
    imageUrl: "URL de imagen",
    date: "Fecha",
    time: "Hora",
    description: "Descripción",
    venue: "Lugar",
    price: "Precio",
    capacity: "Capacidad",
    discount: {
      add: "Agregar descuento",
      type: "Tipo de descuento",
      percent: "Porcentaje",
      amount: "Cantidad",
      value: "Valor",
    },
    soldOut: "Agotado",
    ended: "Finalizado",
    attendees: {
      title: "Asistentes",
      viewAttendees: "Ver asistentes",
      count: "{count} asistentes",
      name: "Nombre",
      bookedAt: "Reservado",
      paymentStatus: "Pago",
      loading: "Cargando asistentes...",
      empty: "Sin reservas aún",
    },
  },

  // Courses
  course: {
    ticketsIncluded: 'Tarjetas de Punche Incluidas',
    ticketsDescription: 'Este curso te da {count} tarjetas de punche que se pueden usar para todas las clases.',
    ticketsExplanation: 'Las tarjetas de punche son flexibles y se pueden usar para cualquier clase, no solo este curso. Vencen al final del período del curso.',
    create: "Crear curso",
    edit: "Editar curso",
    delete: "Eliminar curso",
    deleted: "Curso eliminado",
    saved: "Curso guardado",
    title: "Título",
    imageUrl: "URL de imagen",
    description: "Descripción",
    level: "Nivel",
    levelBeginner: "Principiante",
    levelIntermediate: "Intermedio",
    levelAdvanced: "Avanzado",
    levelNone: "Sin nivel especificado",
    levelPlaceholder: "Seleccionar nivel (opcional)",
    price: "Precio",
    points: "Puntos",
    capacity: "Capacidad",
    instructor: "Instructor",
    status: "Estado",
    statusDraft: "Borrador",
    statusPublished: "Publicado",
    statusArchived: "Archivado",
    empty: "Sin cursos aún",
    pageEditor: {
      editPage: 'Editar Página',
      addSection: 'Agregar Sección',
      saveSection: 'Guardar Sección',
      deleteSection: 'Eliminar Sección',
      moveUp: 'Mover Arriba',
      moveDown: 'Mover Abajo',
      sectionType: 'Tipo de Sección',
      sectionTitle: 'Título de Sección (opcional)',
      confirmDelete: '¿Estás seguro de que quieres eliminar esta sección?',
      hero: 'Hero (portada)',
      text: 'Texto',
      video: 'Video',
      image: 'Imagen',
      gallery: 'Galería',
      faq: 'FAQ',
      booking: 'Inscripción',
      heroDescription: 'La sección hero usará automáticamente el título, imagen, nivel, instructor y fechas del curso.',
      textContent: 'Contenido de Texto',
      textPlaceholder: 'Ingresa contenido de texto...',
      textAlignment: 'Alineación de Texto',
      fontSize: 'Tamaño de Fuente',
      videoUrl: 'URL del Video',
      videoUrlHelp: 'Pega la URL de YouTube o Vimeo, o el código de inserción completo',
      videoUrlExtracted: 'URL del video extraída del código de inserción',
      videoUrlInvalid: 'Formato de URL de video no válido',
      aspectRatio: 'Relación de Aspecto',
      imageUrl: 'URL de Imagen',
      altText: 'Texto Alt',
      caption: 'Leyenda (opcional)',
      galleryImages: 'Imágenes',
      addImage: 'Agregar Imagen',
      faqItems: 'Elementos FAQ',
      question: 'Pregunta',
      answer: 'Respuesta',
      removeItem: 'Eliminar',
      addFaqItem: 'Agregar Elemento FAQ',
      bookingDescription: 'La sección de inscripción mostrará automáticamente el precio del curso, información de tickets, horario de clases y botón de inscripción.',
      titlePlaceholder: 'Ingresa título de sección...',
      editSection: 'Editar Sección',
    },
    beginner: 'Principiante',
    intermediate: 'Intermedio',
    advanced: 'Avanzado',
    instructorTBA: 'Instructor TBA',
    lessons: 'clases',
    enrollmentError: 'Error al crear inscripción',
    includesTickets: 'Incluye',
    tickets: 'tickets',
    enrollNow: 'Inscribirse Ahora',
    lessonSchedule: 'Horario de Clases',
    viewSchedule: 'Ver todas las clases',
    flexibleTicketInfo: 'Sistema de tickets flexibles - usa tickets para cualquier clase',
    ticketExpiry: 'Los tickets vencen',
    notFound: 'Curso no encontrado',
    backToCourses: 'Volver a Cursos',
  },

  // Lessons
  lesson: {
    add: "Agregar lección",
    edit: "Editar lección",
    delete: "Eliminar lección",
    deleted: "Lección eliminada",
    saved: "Lección guardada",
    date: "Fecha",
    start: "Hora de inicio",
    end: "Hora de fin",
    venue: "Lugar",
    note: "Nota",
    empty: "Sin lecciones aún",
  },

  // Calendar
  calendar: {
    lesson: "Lección",
    event: "Evento",
  },

  // Schedule/Schema Page
  schedule: {
    title: "Horario",
    subtitle: "Cursos y eventos 08:00 - 23:00",
    noEventsToday: "No hay clases ni eventos este día",
    time: "Hora",
    day: "Día",
    week: "Semana",
    month: "Mes",
    today: "Hoy",
    next: "Siguiente",
    upcoming: "Próximos",
    colorCoding: "Código de Colores",
    moreItems: "más",
    bookNow: "Reservar ahora",
    viewDetails: "Ver detalles",
    dayDetails: "Resumen del día",
    noBooking: "No se puede reservar",
    weekdays: {
      mon: "Lun",
      tue: "Mar",
      wed: "Mié",
      thu: "Jue",
      fri: "Vie",
      sat: "Sáb",
      sun: "Dom",
    },
  },

  // Profile
  profile: {
    title: 'Mi Perfil',
    personalInfo: 'Información Personal',
    fullName: 'Nombre Completo',
    email: 'Correo Electrónico',
    phone: 'Número de Teléfono',
    profilePicture: 'Foto de Perfil',
    uploadPicture: 'Subir Foto',
    changePicture: 'Cambiar Foto',
    danceExperience: 'Experiencia de Baile',
    selectYourLevel: 'Selecciona tu nivel para cada estilo de baile',
    noExperience: 'Sin Experiencia',
    beginner: 'Principiante',
    intermediate: 'Intermedio',
    pro: 'Avanzado',
    security: 'Seguridad',
    changePassword: 'Cambiar Contraseña',
    enterNewPassword: 'Ingresa tu nueva contraseña',
    newPassword: 'Nueva Contraseña',
    confirmPassword: 'Confirmar Contraseña',
    saveChanges: 'Guardar Cambios',
    success: 'Éxito',
    error: 'Error',
    profileUpdated: 'Perfil actualizado correctamente',
    passwordUpdated: 'Contraseña cambiada correctamente',
    avatarUpdated: 'Foto de perfil actualizada',
    loadError: 'Error al cargar perfil',
    saveError: 'Error al guardar perfil',
    uploadError: 'Error al subir imagen',
    passwordError: 'Error al cambiar contraseña',
    passwordMismatch: 'Las contraseñas no coinciden',
    passwordTooShort: 'La contraseña debe tener al menos 6 caracteres',
    invalidFileType: 'Por favor sube un archivo de imagen',
    fileTooLarge: 'El tamaño del archivo debe ser menor a 5MB',
    leader: 'Líder',
    follower: 'Seguidor',
    notSet: 'No Configurado',
  },

  // CRM
  crm: {
    title: "CRM de Miembros",
    subtitle: "Gestionar miembros y ver resumen de ingresos",
    totalMembers: "Total de Miembros",
    totalRevenue: "Ingresos Totales",
    avgPerMember: "Promedio por Miembro",
    levelDistribution: "Distribución de Niveles",
    search: {
      placeholder: "Buscar por nombre, email o teléfono…",
    },
    filter: {
      level: "Todos los niveles",
    },
    sort: {
      revenue: "Mayores ingresos",
      recent: "Más reciente",
      oldest: "Más antiguo",
      points: "Más puntos",
      tickets: "Más entradas",
      name: "Nombre A–Z",
    },
    members: "miembros",
    table: {
      name: "Nombre",
      contact: "Contacto",
      level: "Nivel",
      points: "Puntos",
      tickets: "Entradas",
      revenue: "Ingresos Totales",
      since: "Miembro Desde",
    },
    view: "Ver",
    level: {
      bronze: "Bronce",
      silver: "Plata",
      gold: "Oro",
      platinum: "Platino",
      vip: "VIP",
    },
    actions: {
      changeLevel: "Cambiar Nivel",
      adjustPoints: "Ajustar Puntos",
      disable: "Desactivar Cuenta",
      enable: "Activar Cuenta",
      email: "Enviar Email",
      edit: "Editar Miembro",
      delete: "Eliminar Miembro",
      add: "Agregar Miembro",
      changeRole: "Cambiar Rol",
      manualCheckin: "Check-in Manual",
      giveEventTicket: "Dar entrada de evento",
      selectEventDate: "Seleccionar fecha",
      allDates: "Todas las fechas",
      tieToCourse: "Vincular a curso (opcional)",
      genericCourse: "Genérico – cualquier clase",
      createFreeBooking: "Crear reserva gratuita",
      generateCompCode: "Generar código",
      compCodeCreated: "Código {code} creado – copia y comparte",
      eventBookingCreated: "Reserva gratuita para {event} creada",
      attendees: "Asistentes",
      ticketTarget: "Objetivo",
      targetDropIn: "Drop-in / genérico",
      targetCourse: "Curso (semestre, principiantes…)",
      targetEvent: "Evento",
      selectCourse: "Selecciona curso",
      selectEvent: "Selecciona evento",
      currentClips: "Paquetes de clips actuales",
      currentEventBookings: "Reservas de evento activas",
      noClips: "Sin clips activos",
      noEventBookings: "Sin reservas activas",
      cancelBooking: "Cancelar reserva",
      bookingCancelled: "Reserva para {event} cancelada",
      confirmCancelBooking: "¿Cancelar esta reserva? La plaza quedará libre.",
      give: "Dar",
      remove: "Quitar",
    },

    drawer: {
      overview: "Resumen",
      purchases: "Historial de Compras",
      tickets: "Entradas y Asistencia",
      subs: "Suscripciones",
      notes: "Notas",
    },
    points: "Puntos",
    total: "Total",
    purchases: "Compras",
    lastActivity: "Última Actividad",
    addNote: "Agregar Nota",
    saved: "Guardado",
    updated: "Actualizado",
    error: "Algo salió mal",
    empty: "Ningún miembro coincide con el filtro.",
    noNotes: "Sin notas aún",
    noteBody: "Nota",
    enterPoints: "Ingresar puntos (+/- para ajuste)",
    selectLevel: "Seleccionar nivel",
    adminGift: 'Regalo Admin',
    freeTicket: 'Gratis',
    selectRole: "Seleccionar rol",
    selectCourse: "Seleccionar curso",
    role: "Rol",
    checkinNote: "Nota (opcional)",
    confirmDelete: "¿Estás seguro de que quieres eliminar este miembro?",
    deleteWarning: "Esta acción no se puede deshacer.",
    addMember: "Agregar Miembro",
    editMember: "Editar Miembro",
    fullName: "Nombre Completo",
    email: "Email",
    phone: "Teléfono",
    password: "Contraseña",
    passwordPlaceholder: "Dejar vacío para mantener actual",
    exportSuccess: 'Miembros exportados a CSV',
    exportError: 'No se pudo exportar a CSV',
  },

  // Event Tickets
  eventTickets: {
    buyTickets: 'Comprar Entradas',
    singleTicket: '1 Entrada',
    coupleTickets: '2 Entradas (Pareja)',
    trioTickets: '3 Entradas',
    save: 'Ahorrar',
    notEnoughSpots: 'No hay suficientes lugares',
    attendeeNames: 'Nombres de Asistentes',
    attendeeNamesDescription: 'Ingrese el nombre de cada persona que asistirá',
    person: 'Persona',
    enterName: 'Ingrese nombre',
    fillAllNames: 'Por favor complete todos los nombres de asistentes',
    buyNow: 'Comprar Ahora',
    couplePrice: 'Precio Pareja (2 entradas)',
    trioPrice: 'Precio para 3 entradas',
    couplePriceHelp: 'Dejar vacío para usar el doble del precio individual',
    trioPriceHelp: 'Dejar vacío para usar el triple del precio individual',
    attendees: 'Asistentes',
    checkinsProgress: 'Check-ins',
  },

  // My Payments
  myPayments: {
    title: 'Mis Pagos',
    subtitle: 'Ve tu historial de pagos y descarga recibos',
    noPayments: 'No se encontraron pagos.',
    downloadReceipt: 'Recibo',
    downloadError: 'No se pudo descargar el recibo',
    statusPaid: 'Pagado',
    statusPending: 'Pendiente',
    navLabel: 'Mis Pagos',
  },
};
